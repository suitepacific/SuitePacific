...
| 51 | runPaged() can give duplicate or missing results if the search isn't sorted correctly — NetSuite requires a unique and unambiguous sort order; sort by Transaction Date + Internal ID, not Transaction Date alone | SuiteScript |
| 52 | runPaged() does not give you a frozen snapshot — records can be created, modified, or deleted while you traverse pages; design for a changing result set, not a stable one | SuiteScript |
| 53 | Your afterSubmit script runs after the record is saved, but related records may not be ready yet — queue/flag in afterSubmit, re-read current state when the background process runs | SuiteScript |
| 54 | A Map/Reduce restart can run your processing again — the framework cleans up its own state but does not undo changes your code made to other records; design map() and reduce() to be idempotent | SuiteScript / Map/Reduce |
| 55 | SuiteQL can return the right transactions and still give the wrong total — one-to-many joins multiply rows; DISTINCT does not fix a join problem; know the grain before aggregating | SuiteQL |
| 56 | A User Event with context.type = EDIT does not mean the field your script cares about actually changed — compare context.oldRecord vs context.newRecord for the specific field before doing expensive work | SuiteScript |
| 57 | A higher Map/Reduce Buffer Size can make a restart repeat more work than expected — default is 1; increasing it is a tradeoff between efficiency and restart risk; leave at 1 for record processing | SuiteScript / Map/Reduce |
| 58 | A Client Script can set a field correctly and still read the wrong value immediately after — sourcing triggered by setValue() can be asynchronous; use postSourcing or forceSyncSourcing when you need the sourced value | SuiteScript / Client Script |
| 59 | A SuiteQL query can fail because the role lacks permission for a field or record it touches — SUITE_QL (default) fails on missing permissions; STATIC succeeds but returns no data for inaccessible fields; STATIC is not a permissions bypass | SuiteQL |
| 60 | N/cache doesn't automatically make a script faster — performance depends on cache hits; keys including Date.now() produce 100% miss rate; PRIVATE scope for shared data means each user runs the loader separately | SuiteScript / Performance |
| 61 | isDynamic: changing it from false to true looks like a one-line change but changes how sourcing, validation, and calculation work — field order matters in dynamic mode because sourcing runs in real time | SuiteScript |
| 62 | Map/Reduce persisted data has a 200 MB total limit — search results from getInputData() count toward it; large result sets that process fine in isolation can fail with PERSISTED_DATA_LIMIT_FOR_MAPREDUCE_SCRIPT_EXCEEDED when serialized | SuiteScript / Map/Reduce |
