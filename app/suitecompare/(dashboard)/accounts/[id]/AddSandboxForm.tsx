...
import { CheckCircle2, Lock, Plus, X, Info } from "lucide-react";

function AddSandboxModal({ accountId, onClose }: { accountId: string; onClose: () => void }) {
...
export function AddSandboxForm({ accountId, planLimited }: { accountId: string; planLimited?: boolean }) {
  ...
  return (
    <>
      ...
      {open && <AddSandboxModal accountId={accountId} onClose={() => setOpen(false)} />}
    </>
  );
}
