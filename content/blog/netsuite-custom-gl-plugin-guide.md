...
<p style="margin:0;color:#14306b;font-size:0.9rem;line-height:1.6">The Custom GL Plugin lets you add new debit and credit lines to a transaction's GL impact when it posts. It does not let you change, delete, or override the lines that NetSuite generates as part of its standard accounting logic. The plug-in receives the standard GL lines as a read-only input and returns additional lines to append. If your requirement is to change what NetSuite would normally post, the Custom GL Plugin is not the right solution. If your requirement is to add supplemental GL activity on top of what NetSuite posts, it is the right solution. The plug-in is deployed under Setup &gt; Accounting &gt; Plug-ins and implemented as a SuiteScript with a customizeGlImpact entry point function that NetSuite calls each time a qualifying transaction posts. One critical constraint: the plug-in does not execute on all transaction types. Intercompany journal entries, statistical journal entries, reversing journal entries created by NetSuite, voiding entries, and period adjustment entries do not trigger it. Testing against each relevant transaction type in Sandbox before Production deployment is essential because a plug-in defect that reaches Production creates posted GL entries requiring manual correction.</p>
</div>
