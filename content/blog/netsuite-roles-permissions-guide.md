...
<p style="margin:0;color:#14306b;font-size:0.9rem;line-height:1.6">A role in NetSuite is a named collection of permissions. Each permission grants access to a specific record type, transaction type, list, report, or setup task. Permissions come in five levels: None, View, Create, Edit, and Full. A user's access is determined entirely by the permissions on their assigned role. Assigning a user to a role with no permissions on Purchase Orders means that user cannot see any purchase orders, regardless of any other setting. Roles are the single source of truth for what a user can and cannot do in NetSuite. Custom roles are created under Setup &gt; Users/Roles &gt; Manage Roles by clicking New or customizing an existing standard role. The built-in Administrator role cannot be customized and grants full unrestricted access to the entire account, including the ability to delete records and modify all configuration. It should be treated as an emergency role, not a default assignment. Users can hold multiple roles simultaneously; their effective permissions are the union of all roles assigned. The Restrictions tab on each role controls access by subsidiary, department, class, or location, applied in addition to permission levels.</p>
</div>
