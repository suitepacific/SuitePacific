...
<p style="margin:0;color:#14306b;font-size:0.9rem;line-height:1.6">A role in NetSuite is a named collection of permissions. Each permission grants access to a specific record type, transaction type, list, report, or setup task at one of five levels: None, View, Create, Edit, or Full. A user's access is determined entirely by the permissions on their assigned role. Roles are the single source of truth for what a user can and cannot do in NetSuite. Custom roles are created under Setup &gt; Users/Roles &gt; Manage Roles. The built-in Administrator role cannot be customized and grants full unrestricted access to the entire account, including the ability to delete records and modify all configuration. It should be treated as an emergency role, not a default assignment. Users can hold multiple roles simultaneously; their effective permissions are the union of all roles assigned. The Restrictions tab on each role controls access by subsidiary, department, class, or location, applied in addition to permission levels.</p>
</div>
