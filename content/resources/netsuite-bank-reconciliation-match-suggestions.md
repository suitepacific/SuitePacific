...
<p style="margin:0;color:#14306b;font-size:0.9rem;line-height:1.6">NetSuite 2026.2 replaced the Review subtab on the Match Bank Data page with a new Match Suggestions subtab. The Match Suggestions subtab presents system-suggested matches between bank transactions and NetSuite records. Five actions are available for each suggestion: Match (accept and link the records), Match All (accept all suggestions at once), Unmatch (break an existing match), Resolve (mark as reconciled without a matching record), and Ignore (dismiss the suggestion without action). The Submit button on this page was also renamed to Update. Navigate to Transactions > Bank > Match Bank Data and select the Match Suggestions tab to access these options. The underlying matching logic is unchanged from previous releases; only the UI layout was reorganized. If your team has documented reconciliation procedures that reference the Review subtab or the Submit button by name, update those procedures after the 2026.2 upgrade reaches your sandbox account.</p>
</div>
