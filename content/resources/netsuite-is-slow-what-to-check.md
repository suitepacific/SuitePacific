...
<p style="margin:0;color:#14306b;font-size:0.9rem;line-height:1.6">Most NetSuite performance problems are caused by account configuration, not the platform. The five most common causes are: dashboard saved searches that reload on every login; custom scripts that run on every record save without execution conditions limiting when they fire; workflows without entry conditions that evaluate on every save of every applicable record; accumulated unused metadata (custom fields, unused scripts, deactivated but still-evaluating workflows); and saved searches that apply criteria after loading all matching records rather than filtering at the database level. Check these five areas before filing a support case with NetSuite. To isolate dashboard-related slowness, temporarily remove all portlets and log back in. If login speed improves significantly, add portlets back one at a time to identify which saved search is the bottleneck, then optimize or remove that search. Unused custom fields still add load time even when hidden, because NetSuite validates their metadata on every relevant record type load.</p>
</div>
