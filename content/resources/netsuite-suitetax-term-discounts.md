...
<p style="margin:0;color:#14306b;font-size:0.9rem;line-height:1.6">NetSuite SuiteTax accounts can now calculate tax on term discounts (early payment discounts) using a feature added in 2026.2. Enabling it requires two settings: first, enable Tax Term Discounts at Setup > Accounting > Accounting Preferences > Tax tab; second, enable the equivalent setting at the nexus level under Setup > Tax > Nexuses, edit the relevant nexus, and enable the term discount tax option. Both settings must be active for the feature to apply on transactions. Test with a purchase transaction in sandbox to confirm that the tax adjustment appears correctly when an early payment discount is applied. The feature posts the tax adjustment to the nexus's configured tax account. If you have multiple nexuses, verify the setting is enabled for each applicable nexus; enabling it at Accounting Preferences does not automatically activate it at the nexus level.</p>
</div>
